...
public class DatabaseDataEngine {
    ...
    public ArrayList<PingData> getPingDataBetweenDates(Timestamp startDate, Timestamp endDate){
        ...
        try{
            ...
            databaseEngine.addLog("DatabaseDataEngine", "Successfully got ping data between dates " + startDate + " and " + endDate + " with " + pingData.size() + " rows", "INFO", "#00FF00");
            return pingData;
        }
        catch(SQLException e){
            databaseEngine.addLog("DatabaseDataEngine", "Error getting ping data between dates " + e.getMessage(), "ERROR", "#FF0000");
            return null;
        }
    }
